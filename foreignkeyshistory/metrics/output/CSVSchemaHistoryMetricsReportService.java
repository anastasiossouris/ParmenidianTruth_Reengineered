...
//		fill in the rest
		for(int i=1;i<columns;i++)
		{
				if(i==1)
				{
					report[1][i] = getSchemaMetricsCalculator(diachronicSchema).getNumberOfConnectedComponents() + ",";
				}
				else
				{
					report[1][i] = getSchemaMetricsCalculator(schemaHistory.getSchemaVersion(i-2)).getNumberOfConnectedComponents() + ",";
				}
		}
...
//		fill in the rest
		for(int i=1;i<columns;i++)
		{
			if(i==1)
			{
					report[1][i] = getSchemaMetricsCalculator(diachronicSchema).getForeignKeyCount() + ",";
			}
			else
			{
					report[1][i] = getSchemaMetricsCalculator(schemaHistory.getSchemaVersion(i-2)).getForeignKeyCount() + ",";
			}
		}
...
//		fill in the rest
		for(int i=1;i<columns;i++)
		{
				if(i==1)
				{
					report[1][i] = getSchemaMetricsCalculator(diachronicSchema).getForeignKeyCountForGcc() + ",";
				}
				else
				{
					report[1][i] = getSchemaMetricsCalculator(schemaHistory.getSchemaVersion(i-2)).getForeignKeyCountForGcc() + ",";
				}
		}
...
//		fill in the rest
		for(int i=1;i<columns;i++)
		{
				if(i==1)
				{
					report[1][i] = getSchemaMetricsCalculator(diachronicSchema).getTableCount() + ",";
				}
				else
				{
					report[1][i] = getSchemaMetricsCalculator(schemaHistory.getSchemaVersion(i-2)).getTableCount() + ",";
				}
		}
...
//		fill in the rest
		for(int i=1;i<columns;i++)
		{
				if(i==1)
				{
					report[1][i] = getSchemaMetricsCalculator(diachronicSchema).getTableCountForGcc() + ",";
				}
				else
				{
					report[1][i] = getSchemaMetricsCalculator(schemaHistory.getSchemaVersion(i-2)).getTableCountForGcc() + ",";
				}
		}
...
//		fill in the rest
		for(int i=1;i<columns;i++)
		{
			for(int j=1;j<lines;j++)
			{
				if(i==1)
				{
					report[j][i] = getSchemaMetricsCalculator(diachronicSchema).getTableOutDegree(diachronicSchema.getTableWithName(getTableNameFromReport(report, j))) + ",";
				}
				else
				{
					Schema schema = schemaHistory.getSchemaVersion(i-2);
...
//		fill in the rest
		for(int i=1;i<columns;i++)
		{
			for(int j=1;j<lines;j++)
			{
				if(i==1)
				{
					report[j][i] = getSchemaMetricsCalculator(diachronicSchema).getTableInDegree(diachronicSchema.getTableWithName(getTableNameFromReport(report, j))) + ",";
				}
				else
				{
					Schema schema = schemaHistory.getSchemaVersion(i-2);
...
//		fill in the rest
		for(int i=1;i<columns;i++)
		{
				if(i==1)
				{
					report[1][i] =  getSchemaMetricsCalculator(diachronicSchema).getSchemaDiameter() + ",";
				}
				else
				{
					report[1][i] = getSchemaMetricsCalculator(schemaHistory.getSchemaVersion(i-2)).getSchemaDiameter() + ",";
				}
		}
...
//		fill in the rest
		for(int i=1;i<columns;i++)
		{
			for(int j=1;j<lines;j++)
			{
				if(i==1)
				{
					Schema schema = diachronicSchema;
					...
					report[j][i] = getSchemaMetricsCalculator(diachronicSchema).getForeignKeyBetweenness(foreignKey) + ",";
				}
				else
				{
					Schema schema = schemaHistory.getSchemaVersion(i-2);
...
//		fill in the rest
		for(int i=1;i<columns;i++)
		{
			for(int j=1;j<lines;j++)
			{
				if(i==1)
				{
					report[j][i] = getSchemaMetricsCalculator(diachronicSchema).getTableBetweenness(diachronicSchema.getTableWithName(getTableNameFromReport(report, j))) + ",";
				}
				else
				{
					Schema schema = schemaHistory.getSchemaVersion(i-2);
...
//		fill in the rest
		for(int i=1;i<columns;i++)
		{
			for(int j=1;j<lines;j++)
			{
				if(i==1)
				{
					report[j][i] = getSchemaMetricsCalculator(diachronicSchema).getTableDegree(diachronicSchema.getTableWithName(getTableNameFromReport(report, j))) + ",";
				}
				else
				{
					Schema schema = schemaHistory.getSchemaVersion(i-2);
	...
	private String getSourceFromStringForeignKey(String foreignKey)
	{
		...
		return foreignKey.substring(0, index-1);
	}
